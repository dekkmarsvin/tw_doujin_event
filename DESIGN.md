...
整體採明亮、低彩度、紙張感的工具外殼；高密度不是壓縮一切，而是建立固定欄位、可預測的層級與精準的資料字體。桌機以探索／行程共用的左欄與向量地圖兩區為主、詳情條件式覆蓋（見 Two-Zone Rule），手機則以探索／行程兩入口與獨立社團摘要，讓地圖與當前工作保持在同一畫面。
...
- 網站管理集中在 `/admin`，與參觀者及社團編輯流程分離；活動核准與發布留在 `/organizer` 的活動脈絡。
...
- **薄荷色** (`mint-illustration`): 分類色盤的一色，以及成功狀態的核心色。
- **藍** (`blue-vtuber`): 分類色盤的一色，以及入口方向資訊。
- **琥珀色** (`amber-maker`): 分類色盤的一色，以及警告語意。
- **丁香色** (`lilac-cosplay`): 分類色盤的一色。

分類色依主辦分類目錄的順序循環套用，不綁定特定分類名稱；分類多於色數時顏色會重複，辨識以文字標籤為準。token 名稱沿用舊命名，不代表對應的分類。
...
桌機頂部提供 100%、112%、124% 三段介面字級；倍率套用至文字而不放大 SVG 地圖幾何。760px 以下維持 100%，避免把有限的地圖視野交給介面字級控制，同時仍支援瀏覽器原生縮放。

## Layout

桌機工作區採探索／行程共用左欄與 SVG 地圖兩區；詳情選取時覆蓋地圖右側，不占 grid 欄位。>1050px 左欄 296px，761–1050px 為 248px，選取後暫時收起左欄（恢復與切換行為見[活動地圖契約](docs/contracts/event-map.md)）。詳情寬至多 330px，頂與右側 16px，高度隨內容縮合，最長保留底部 76px；標頭、收藏與行程操作留在上方，圖片及補充內容獨立捲動。760px 以下使用探索／行程兩入口，工作面板可收合、半展開約 44dvh、完整約 82dvh；選取社團改用約 254px 摘要，短螢幕 210px。字級、資料管理與使用說明收納至手機工具選單；對話框 portal 到文件根層。

桌機 header 基準 72px，活動名稱、日期與場館依內容分配寬度並容許換行，窄螢幕或放大字級時搜尋獨立一列；多場活動時名稱整區附「切換活動」連結。桌機字級群組與相鄰閱讀工具採 44px 高、7px 圓角。shell 使用 100dvh 與 auto/minmax(0,1fr) 列高，按實際 header 高度配置剩餘空間。篩選最多占左欄內容 45%，結果及行程獨立捲動。

**The Two-Zone Rule.** 探索與行程共用左欄，地圖保留剩餘空間，詳情條件式覆蓋。首次完整場館 fit，固定工具遮蔽納入 fit；詳情只影響選取定位。見 [ADR-0055](docs/adr/0055-desktop-map-uses-two-zones-and-overlay-details.md)。

**手機工作入口與摘要分開。** 依 ADR-0056 及 ADR-0063，篩選位於探索內，底部只有探索／行程。選取後摘要左上回結果、右上取消選取，代碼與名稱並列、介紹最多兩行、三個緊湊動作。取消選取、收合與關閉的行為見[活動地圖契約](docs/contracts/event-map.md)。短螢幕最大字級省略介紹及兩個規劃快捷動作，保留完整資訊入口。
...
- **詳情浮層** (`0 14px 40px #2631293a`): 桌機條件式覆蓋地圖的社團詳情。
- **對話框** (`0 28px 80px #11182066`): 完整資訊、資料管理與主辦工作區等離開頁面的對話框。
...
- **Do** 使用桌機雙區與條件式詳情覆蓋，761–1050px 選取後暫時收起左欄，760px 處重組一般使用者流程。
...
- **Do** 為外部內容顯示提供者與資料時間，有原始連結時附「原始來源」。
