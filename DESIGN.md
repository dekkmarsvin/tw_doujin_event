...
桌機工作區採探索／行程共用左欄與 SVG 地圖兩區；詳情選取時覆蓋地圖右側，不占 grid 欄位。>1050px 左欄 296px，761–1050px 為 248px；頁籤與導航切換不改欄寬。詳情寬至多 330px，頂與右側 16px、底部 76px，內容獨立捲動。760px 以下使用探索／行程兩入口，工作面板可收合、半展開約 44dvh、完整約 82dvh；選取社團改用約 254px 摘要，短螢幕 210px。字級、資料管理與使用說明收納至手機工具選單；對話框 portal 到文件根層。
...
- **Do** 使用桌機雙區與條件式詳情覆蓋，1050px 處縮窄左欄，760px 處重組一般使用者流程，720px 處重組管理面板。
