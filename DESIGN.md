...
桌機 header 基準 72px，窄螢幕或放大字級容許換行；shell 使用 100dvh 與 auto/minmax(0,1fr) 列高，按實際 header 高度配置剩餘空間。篩選最多占左欄內容 45%，結果及行程獨立捲動。
...
**手機工作入口與摘要分開。** 依 ADR-0056，篩選位於探索內，底部只有探索／行程。選取後摘要左上回結果、右上收起，代碼與名稱並列、介紹最多兩行、三個緊湊動作。收起保留 URL 與視域；完整資訊使用原有 dialog。短螢幕最大字級省略介紹及兩個規劃快捷動作，保留完整資訊入口。
