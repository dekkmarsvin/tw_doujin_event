...
最終方向種子為 `local-extension-c`。整體採明亮、低彩度、紙張感的工具外殼；高密度不是壓縮一切，而是建立固定欄位、可預測的層級與精準的資料字體。桌機把規劃工具、向量地圖、詳細資訊／行程並置，手機則以探索／行程兩入口與獨立社團摘要，讓地圖與當前工作保持在同一畫面。
...
桌機工作區採探索／行程共用左欄與 SVG 地圖兩區；詳情選取時覆蓋地圖右側，不占 grid 欄位。>1050px 左欄 296px，761–1050px 為 248px，選取後暫時收起左欄，回搜尋／回行程恢復原清單、捲動及焦點；頁籤與導航切換不改欄寬。詳情寬至多 330px，頂與右側 16px，高度隨內容縮合，最長保留底部 76px；標頭、收藏與行程操作留在上方，圖片及補充內容獨立捲動。760px 以下使用探索／行程兩入口，工作面板可收合、半展開約 44dvh、完整約 82dvh；選取社團改用約 254px 摘要，短螢幕 210px。字級、資料管理與使用說明收納至手機工具選單；對話框 portal 到文件根層。

桌機 header 基準 72px，活動名稱、日期與場館依內容分配寬度並容許換行，窄螢幕或放大字級時搜尋獨立一列；多場活動時名稱整區附「切換活動」連結，桌機與手機均可返回選擇頁。桌機字級群組與相鄰閱讀工具採 44px 高、7px 圓角。shell 使用 100dvh 與 auto/minmax(0,1fr) 列高，按實際 header 高度配置剩餘空間。篩選最多占左欄內容 45%，結果及行程獨立捲動。
...
**手機工作入口與摘要分開。** 依 ADR-0056 及 ADR-0063，篩選位於探索內，底部只有探索／行程。選取後摘要左上回結果、右上取消選取，代碼與名稱並列、介紹最多兩行、三個緊湊動作。取消選取清除社團及攤位 URL 並保留視域；把手收合保留選取，完整資訊關閉返回摘要。短螢幕最大字級省略介紹及兩個規劃快捷動作，保留完整資訊入口。
...
- **Do** 使用桌機雙區與條件式詳情覆蓋，761–1050px 選取後暫時收起左欄，760px 處重組一般使用者流程，720px 處重組管理面板。
