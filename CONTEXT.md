...
| **layout** | 辨識後的向量資料（排、slot、柱子、出入口、服務設施） |
...
**服務設施**（service point）
場內讀者會找的服務：廁所、無障礙廁所、服務台（含大會本部）、寄物處、醫護站、樓梯、電梯。保存一個點與固定的類型，名稱選填，用來區分同類的幾處。它掛在每份 layout，不是場館參考資料：座標是各自配置圖的像素空間。編輯器把柱子、出入口與非一般攤位區的放置工具統稱「設施」，讀者端的「設施」清單則同時列出出入口、服務設施與具名區域；「服務設施」只指這七類。
