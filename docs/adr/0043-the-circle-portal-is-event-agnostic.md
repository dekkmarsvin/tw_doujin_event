...
**讀者端**——這是唯一一項會靜默壞掉、也是唯一擋著 [#119](https://github.com/dekkmarsvin/tw_doujin_event/issues/119) 的：

| 現在 | 決策落地後 |
|---|---|
| 只有 `EVENT_ID` 那場活動看得到「由社團填寫」的內容 | 每一場已發布活動都看得到自己的社團補充資料 |

沒做的話，第二場活動的讀者看到的是一份**只有主辦資料的薄場刊**，而且沒有任何錯誤訊息告訴他們少了東西。

**社團端**：

| 現在 | 決策落地後 |
|---|---|
| `/circle` 只服務當期活動，換活動要維護者改設定並重新部署 | 一個入口涵蓋所有活動 |
| 登入後直接進入唯一那場活動 | 登入後選場次；只在一場有認領的人直接進去，不多問 |
| 上一場活動的補充資料在換活動後無從維護 | 每一場的認領與內容各自獨立，都還在 |

**社團會遇到的一件事**：在 FF47 認領過，下一場活動仍要**重新認領並提出該活動的證據**。這是決策 3 的明知代價，不是遺漏——認領證明的是「你是這場活動的這個攤位」。介面可以提示「你在 FF47 認領過同名社團」來省去搜尋，但不會因此免除證據。

**管理者端**：撤下、認領審核與逾期草稿報表會同時看到多場活動的資料，因此清單必須自己帶活動維度——否則兩場的同名社團在一份清單裡分不出來。這是新增的責任，不是自動變好。

**沒有變的**：真人驗證、magic link、session 期限、自助刪除、活動後退出與保存期限的行為都不變。保存期限已經存在資料列上（[ADR-0018](./0018-retention-is-the-circles-choice.md)），不會因為多活動而重算。
