...
1. 恢復只適用於目前核准版本的 failed AMEND：data PR 已合併，main 尚未合併，沒有 deployment workflow checkpoint，且原公開基準仍有效。不延伸成 CREATE、已公開版本或任意步驟的回滾。
2. 維護者在 data repo 建立獨立還原 PR，經既有 `data / check` 後以單一 parent 的 squash commit 合併。內容只把該活動目錄還原至原公開 pin，其他活動與共用 reference 不得被還原。原 main PR 必須關閉且未合併；原 PR、branch、checkpoint 與 snapshot 均保留。
