...
`allocations.json`／`evidence.json` 與該活動的 pin 放在同一張 main PR（[社團資料更新 runbook](../runbooks/catalog-data-update.md) 步驟 2–3）。因此成本是二元的，不是漸進的：

| 時機 | 修正成本 |
|---|---|
| main PR 合併前 | 近乎零。沒有讀者收藏、沒有分享連結、沒有認領指向這些 ID，砍掉重跑即可 |
| main PR 合併後 | 高。收藏與行程存在使用者自己的瀏覽器（[ADR-0002](./0002-planning-data-stays-on-device.md)），分享連結帶著 `selectedCircle=c-…`，兩者都不在本站控制範圍內 |
...
fail closed 本身是對的：這些情況需要人工判讀，自動處理會讓永久 ID 悄悄改變指向。缺的是**判讀之後該做什麼**。處置記在 [#139](https://github.com/dekkmarsvin/tw_doujin_event/issues/139)，本 ADR 不預先決定形狀。
