...
決策段是取捨，這一段是**讀者實際會看到什麼不一樣**。

| 現在 | 決策落地後 |
|---|---|
| 打開 `map.kotoban.top` 直接進入 FF47，沒有別的選擇 | 看到已發布活動清單，選一場進入 |
| 第二場活動上線時 FF47 被取代，舊分享連結進到新活動或回預設 | 舊連結永遠回到 FF47 原本的畫面 |
| `?event=` 帶其他活動會整份 fail closed 回預設 | 帶已發布活動就進入該活動；只有未發布的才 fail closed |
| 「當期活動」是 build 時決定的 | 「看哪一場」是讀者決定的 |

**沒有變的**：進入某一場活動之後，搜尋、地圖、收藏、行程與詳細搜尋全部與現在相同。收藏與行程本來就按 `eventId` 分區（[收藏與走訪規劃契約](../contracts/planning.md)），所以在活動之間切換不會互相污染，也不需要使用者做任何遷移。

**批次二（[#134](https://github.com/dekkmarsvin/tw_doujin_event/issues/134)）再往上加的**：首次進入時的引導、以及活動依即將舉辦／正在舉行／已經結束分組。批次一只給清單，不給分層——兩場活動的清單不需要分層就能用。

**不會出現的**：跨活動的搜尋或收藏彙整。選了一場活動就是在那場活動裡，這不在本 ADR 範圍。
