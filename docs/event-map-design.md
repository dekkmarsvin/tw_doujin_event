...
1. 使用者選擇活動、日期與區域；只有多館或多層活動需要先顯示場館總覽。
2. P1 使用者以拖曳、滑鼠滾輪、觸控手勢或固定控制器平移和縮放；「重設」回到完整可用範圍。
3. 使用者可搜尋攤位代碼或社團名稱，選取結果後地圖直接移動並放大到對應攤位。
4. 點選攤位後同步高亮 SVG slot、更新 URL，並開啟相同社團資料的地圖側欄；共用攤位以緊密清單切換社團。
...
1. 管理員開啟「管理地圖」。
2. 上傳 FF47 配置圖，在瀏覽器完成純像素辨識。
3. 預覽 SVG 草稿與辨識摘要；原圖只在此階段作比較。
4. 信心符合門檻後按「發布活動地圖」。
5. route 驗證並 UPSERT 到 D1，回傳 revision。
6. 所有使用者重新載入時從 GET 取得同一份活動地圖。
