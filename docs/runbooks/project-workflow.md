...
2026-09-21 核對 GitHub open issues 與 main `c6431a7`。目前沒有 open PR。以下是**尚待執行的順序**，不是完成紀錄。每次開工核對新 PR／issue 決策，跳過已有完成證據的工作。
...
**下一張實作票仍是 #221，但只剩兩項。** PR #315 已交付 11 項完成定義中的 9 項；這張票維持開啟，剩下的範圍見下表與 issue 留言。
...
| --- | --- | --- | --- | --- |
| 1 | **#221（續）：待修正清單的顯示時機與其餘 CTA** | PR #315 已合併 | **實機操作** | 只剩兩項：**Phase 4.5** 讓完整 blocking 清單集中在「檢查與預覽」與送審時顯示，側欄目前仍恆常顯示最多 5 項；**Phase 5 其餘 CTA**——匯入空狀態、「重新檢查」、「預覽活動」與送審區的具體動作。Phase 1–3、4.1–4.3 與地圖空狀態已完成。 |
