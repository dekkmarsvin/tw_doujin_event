...
2026-09-21 核對 GitHub open issues 與 main `8f7d336`。目前沒有 open PR。以下是**尚待執行的順序**，不是完成紀錄。每次開工核對新 PR／issue 決策，跳過已有完成證據的工作。
...
**下一張實作票是 #294 → #298（展區的填寫依據）。** #294 的第 3 項（是否修正 `pf45-rf14` 已發布資料）仍待維運者決定，前兩項不受影響可先做。
...
**#221 於 2026-09-21 完成並移出本表**（PR #315 交付 9 項，PR #317 交付最後的 Phase 4.5 與 Phase 5）。4.5 採用的規則是：側欄只列出「已經做了但有問題」的區段，尚未開始的工作不在那裡出現，完整的 blocking 清單留給〈檢查與預覽〉——在那裡它是被要求的，而不是跟著每一個畫面走。正在編輯的區段例外。

**#221 的 Phase 1–3、4.1–4.3 與地圖空狀態**於 2026-09-21 交付（PR #315），這張票仍留在表上。值得記下的是票面內部的一處拉扯：4.1 要任務第一次打開時中性，4.2 要未完成就不能前進；用停用按鈕同時滿足兩者會留下一個沒有理由的灰按鈕，那是 #217 立下的規則禁止的。採用的做法是讓按鈕可按、讓按下去發生、然後拒絕前進，把理由放在按下去之後。另外 Phase 2 有一項實質行為改變：`nextOrganizerEventDay()` 對第一天不再預設今天，因為那會讓沒人回答過的欄位看起來已回答、那一步直接算完成。
...
| --- | --- | --- | --- | --- |
| 1 | **#294 → #298：展區的填寫依據** | #294 的 `pf45-rf14` 修正與否需維運者決定；#298 的形式待定 | **視覺確認** | #294 讓匯入預覽偵測「每一列的展區值都等於攤位代碼第一個字」並提醒，且讓「無分區」在沒有合適來源欄時直接可選；#298 補上場館／使用空間／展區三層在做選擇當下的說明，形式（示意圖、真實活動舉例或互動 demo）先定案再實作。兩者互補：#298 讓人一開始就填對，#294 在填錯時擋住。 |
| 2 | **#214 → #215：攤位清單編輯器（二）（三）** | #213 已關閉，blocked-by 解除；#215 與 #286 同改 `map-layout-editor.tsx`，不並行 | **實機操作** | #214 讓攤位清單本身成為可維護對象：群組拆分、合併與逐列編輯，整份清單以 `expectedVersion` PUT 回去，維持「匯入是取代語意」不做逐列 API；#215 讓 `MapLayoutEditor` 接收 authoring scope，清單與地圖雙向對照，主辦在畫地圖當下就知道還差哪些攤位。 |
