...
2026-09-20 核對 GitHub open issues（28 張）、#212／#104 最新留言與本機 main `5860b5f`。目前沒有 open PR，工作樹乾淨。以下是**尚待執行的順序**，不是完成紀錄。每次開工核對新 PR／issue 決策，跳過已有完成證據的工作。
...
**下一張實作票是 #286。** 第 0 梯的四項判定已於 2026-09-20 執行：#242、#228、#279 驗收後結案，#104／#212 驗收不通過而維持 OPEN，詳見下節。
...
| **#104／#212** | **不通過。** #104 的 45 項中 44 項已勾，未勾的是「完整建立至發布流程沒有 production code diff、Organizer Git／CLI 操作或 AI agent 依賴」；#212 的 55 項中 5 項未勾，全是第 6 節的人工技術操作指標（production code diff、手動 merge、CLI、AI agent、手動 deployment 各為 0）。實測為非零，逐項數字見 `docs/design/ch20-first-publication-acceptance.md`〈人工操作與完成邊界〉。 | **維持 OPEN，維護者已決定安排一次零補救重測**（2026-09-20）。見下節〈零補救重測〉。不以勾選或刪除指標的方式結案。 |

### 零補救重測

#104 與 #212 唯一未達成的是第 6 節的人工技術操作指標。維護者於 2026-09-20 決定以**一次零補救的完整旅程重測**取代直接結案：在一場全新真實活動上，從建立走到 Reader，全程七項指標為 0。

**重測對象：PF45（花博爭豔館）。** 維護者於 2026-09-20 提供配置圖與攤位 CSV。CH20 已發布，不在統計區間內，不能重跑它充數；preview 環境與合成活動也不算。時程未定前本項保持待排，不阻擋佇列中的其他工作。

**資料形狀（2026-09-20 以提供的 CSV 核對，未匯入）：** 2 個活動日、1 個使用空間（主場館）、13 個區塊（A–D、M–T、W）。活動日 1 共 366 群組／560 碼，活動日 2 共 351 群組／534 碼；日內無重複代碼，多碼群組以逗號分隔（最長四碼，如 `S01,S02,S03,S04`），對應匯入器的 `delimited` 模式。**兩個活動日的地圖不同**：Q 區在日 1 為完整 1–48，在日 2 只有 13–36，其餘位置是「鋼普拉素組體驗」區；因此要畫兩張地圖，不能共用一張。

**RF14 的歸屬已定案（2026-09-20，維護者）：** CSV 檔名雖為 `pf45-rf14-booths.csv`，RF14 是**同一場活動的區塊**，不是獨立活動。整份資料建立為單一 eventId 的候選，13 個區塊同屬一個使用空間，不拆成兩個活動、不新增第二組活動隔離邊界。

**資料由 Organizer 於 UI 匯入與繪製。** 代為匯入、代為產生 layout 或代為修資料都會讓「AI agent 必要操作」不為 0，使該次重測失效；事前的格式核對與缺陷修復不在統計區間內，可以做。

#### 為什麼上次是非零

七項指標的非零值全部來自旅程中途才發現的兩個工程缺陷，不是流程本身需要人工：PR #265（driver 斷言會清掉既有 events）與 PR #266（dispatch 不相容 workerd，釘住的 publication 無法恢復）。為了修它們才產生 production code 修改、2 次工程 PR 人工 merge、6 次 CLI 寫入、AI agent 介入與 4 次人工部署操作。amendment 路徑另有 PR #277。**三者都已在 main**（`14d8edc`、`22daf8f`、`2d66fbe`），同一組原因不會再出現。

#### 進場條件

1. production Pages 與 publication Worker 均已部署到含上述三項修復的版本；執行前實際核對部署版本，不以「已合併進 main」代替。
2. `ORGANIZER_PUBLICATION_MODE` 維持版本控制中的 `github`（`wrangler.jsonc`），不在 dashboard 臨時切換。
3. 會在旅程中途逼出工程介入的已知缺陷先修完：**#286**（排段拖曳無法穩定操作，直接卡在畫地圖那一步）、**#222**（未選場館靜默存第一個場館，錯的場館空間會進送審快照，發現時整段重做）、**#218**（零攤位地圖可存檔）。其餘 UX／文案票不列為前置——它們增加操作成本，但不會逼出 production code 修改。

#### 統計規則

沿用第 6 節，不放寬：統計區間是「系統建置完成後，正常新增這一場活動」；必要 UI 輸入、送審與「核准並發布」不算額外人工發布，系統自動 Git／部署不算人工操作。

旅程中途若必須修 production code、手動 merge、跑 CLI、動用 AI agent 或手動部署，**該次重測即為未通過**：當場記錄原因並開票，不得把修正併入本次統計後宣稱通過。未通過的重測照第 6 節格式留紀錄，修正後另行安排下一次，不作廢也不重寫上一次的數字。

第 6 節另要求的「一次可恢復失敗」已由首次發布的三次真實 UI retry 達成，重測不需要再製造一次失敗；自然發生則照實記錄，刻意安排的故障測試依第 6 節另列，不計入本次統計。

#### 離開條件

- 七項人工技術操作指標實測為 0，第 6 節驗收紀錄完整，含 Reader 實際日期／場館／攤位對照與既有活動（`ff47`、`ch-20`）回歸結果。
- 達標後依第 1 節逐項核對 #104 全部完成條件並結案，#212 的五項指標才可勾選。
- 未達標則兩票維持 OPEN，把當次逼出補救的缺陷排進佇列，重新安排下一次。

### 實作佇列

| 順序 | 工作／歸屬 | 必要前置 | 需視覺確認 | 本次交付與離開條件 |
| --- | --- | --- | --- | --- |
| 1 | **#286：地圖描摹回歸與多排邊界同步** | 無 | **實機操作** | 先修 A／B／C 三項回歸（單排四角縮放跳動、拖動後保留舊外框、描摹模式失去中央命中），再做 D 多排共同上下邊界與格高；可分修復與功能兩個小 PR。A 必須觀察連續 pointer move 的中間畫面、幾何與捲動，不能只比對 pointerup 後的最終 rect；B／C 走真實滑鼠路徑。依 #205 擴充既有 `tests/browser/map-authoring-trace.mjs`，不新增讀取 TSX／CSS 字串的 source-regex 測試。 |
| 2 | **#240／#239／#233：契約索引與型別漂移** | 無 | 不需要 | 三張皆零行為變更，可併一個 PR。#240 把兩個 publication 模組補進 `organizer-workspace.md` 的實作清單並確認 `INDEX.md` 對映；#239 將 `publication.started`（必要時含 `failureCode`／`retryable`）改必填，讓編譯器接手漏填；#233 移除過時的「47 份」並統一 0046 的狀態欄格式。純文件／型別變更依 review-loop 以內容與連結檢查驗證。 |
| 3 | **發布治理重新評估：#232、#227（乙）、#234** | ADR-0058 §5 的觸發條件已成立 | **視覺確認**（僅 #234 文案落點） | 先做一次決策再實作，三者共用同一份決策，不分三次排。ADR-0058 §5 要求前提改變時重新評估是否把 ruleset 恢復為第二道強制邊界，並一併解決「一般人類 PR 如何滿足 `Organizer publication approval`」。#232 補上 PR 作者的本 App 身分綁定（ADR-0058 決策 1 記錄的已知缺口）；#227（乙）決定哪些 `actor_type` 進 `problems`、用同一或分開的 code、以及是否為硬性啟用前置；#234 決定 gate 關閉時失敗文案與停用重試鍵不再各說各話。 |
| 4 | **#224：organizer-app.tsx 拆檔** | 無 | **視覺確認**（重構回歸） | 拆 1660 行單檔與 `ReviewPanel` 的五件事。共用的 `act()` 與 `owner &&` 渲染判斷是第 5、6 步的直接障礙，先拆才做得下去；票標 P3，但位置在依賴鏈的頭。只搬移與拆分，不順帶改文案或行為。 |
| 5 | **#220：回饋落點與清除時機** | #224 | **實機操作** | 每個動作的結果回到發動它的控制項旁，通知隨動作與區段切換清除；不再出現「已儲存。」與「尚有未儲存變更」同框，或按下「建立預覽」成功後頂端仍停在兩步之前的訊息。落點需實際量測，不以截圖目測代替。 |
| 6 | **#217：條件未滿足的動作改停用＋原因** | #224 | **視覺確認** | 在 `docs/design/components.md` 增加這條並與既有 Locked Fields 並列、說明兩者差別；盤點主辦工作區目前以「不渲染」處理條件的控制項並逐一改為停用＋原因，已知至少有 `ReviewPanel` 的送審、協作者與負責人三塊。面板共用的缺失條件依回饋規則在面板頂端講一次。 |
| 7 | **#222：場館必須明確選擇** | 無，可在等待 review 時插入 | **實機操作** | 「請選擇場館」成為真正的預設值，新增使用空間時不預選任何場館與空間。**原表「降低重建 CH20 地圖風險」的選入理由已失效**，改以「場館空間決定地圖張數、匯入能否落地與公開網址形狀，而錯誤選擇是合法值、驗證抓不到」為理由；仍屬 SHOULD，不新增硬性發布 gate。需實際操作「新增使用空間」驗證。 |
| 8 | **#218／#219：地圖與場館表單的兩個落差** | 無 | **視覺確認** | #218 擋下零攤位地圖的建立，不讓空白地圖推進一個版本並在歷史留下空紀錄，同時檢討「1 張地圖」這個計數；#219 讓空間來源網址可繼承場館網址，並把兩個網址欄位的必填錯誤從瀏覽器原生泡泡改為全站一致的行內錯誤、統一命名。 |
| 9 | **#223 → #225：工作區文案收斂** | #223 的決策先於 #225 | **視覺確認** | #223 先決定 `map-contribution-draft` 的問題文案是共用或分流——它同時服務主辦工作區與地圖貢獻兩個控制面，`period` 這類實作術語才會有單一修法；#225 再依 `docs/design/copy.md` 的 Removal Test 收斂內部詞彙（`ALL`）、自相矛盾的狀態（「不會上傳」與「尚未上傳」同框）與冗餘徽章。 |
| 10 | **#221：主辦工作區 UX 整體收斂** | 第 4–9 步 | **實機操作** | 範圍最大的一張：移除頂部六步驟列、引導期不顯示 `ReadinessRail`、修正完成計數、收斂資料結構語言與 CSV 範例入口。前面幾項落地後再做，避免重工。不重做 organizer 架構，不新增第二套 Wizard／進度元件。 |
| 11 | **#214 → #215：攤位清單編輯器（二）（三）** | #213 已關閉，blocked-by 解除；#215 與 #286 同改 `map-layout-editor.tsx`，不並行 | **實機操作** | #214 讓攤位清單本身成為可維護對象：群組拆分、合併與逐列編輯，整份清單以 `expectedVersion` PUT 回去，維持「匯入是取代語意」不做逐列 API；#215 讓 `MapLayoutEditor` 接收 authoring scope，清單與地圖雙向對照，主辦在畫地圖當下就知道還差哪些攤位。 |
| 12 | **零補救重測：#104／#212 結案前置** | 第 1、7 步（#286、#222）與 #218；**需指定一場真實活動** | **實機操作** | 見上節〈零補救重測〉。全程七項人工技術操作指標為 0 才算通過；中途需要工程介入即為未通過，照實記錄後另排下一次。這是 #104／#212 的結案條件，不是新的產品功能。 |
