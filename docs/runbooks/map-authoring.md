...
- **批次操作**：多選後可一起拖曳、以方向鍵微調，或用 inspector 的「自動對齊」與靠左／靠右／靠上／靠下對齊。整組共用同一位移，因此碰到畫布邊界時整組一起停下，不會壓扁彼此間距。一次批次操作是一個復原步驟。
  - **對齊是往該側靠攏，不是疊在同一條線上**：每個元素往指定的那一邊滑，滑到整組外接矩形的那一側邊界、或撞到已經定位的元素為止。另一軸完全不動，所以直排仍是直排、橫排仍是橫排；改變的是間距收攏，而且**兩個攤位不會疊在一起**。並排的元素彼此擋不住，因此一排攤位按靠上就是單純的靠上對齊；同一條線上的元素則會依序緊貼排好。原本互相重疊的一組攤位被推開後會比原本的外接矩形長，末端可以超出該矩形，只有畫布邊界會擋下它。
  - **自動對齊**：一鍵把手繪描出來的一塊整理成格狀——先把每格改成組內最多元素採用的大小（兩種大小一樣多時取較大的，每格以自身中心縮放，出入口沒有大小維持不動），再往上靠攏，最後往左靠攏。順序是關鍵：先往上靠攏讓每一直排各自貼齊上緣，格數與尺寸相同的兩排就會落在同樣的橫列上；有了橫列，往左靠攏才會讓第二排緊貼第一排，而不是壓在上面。兩排直排一起選取後按一次就會變成緊密貼合、不重疊的格狀。
