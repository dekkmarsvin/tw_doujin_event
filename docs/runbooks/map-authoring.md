...
- **「新增排／排段」在原圖上框出一段攤位**：按下按鈕後直接在底圖拖曳，框住一段連續攤位，再填排標籤、代碼前綴、起始編號、格數、補零位數、方向與**編號起點**（直排：上→下／下→上；橫排：左→右／右→左）。**座標與每格寬高由框選結果算出**，一般流程不必輸入。框選矩形就是這一段的**總外框**：直排 N 格時每格高為框高的 N 分之一、寬即框寬，橫排反之。每格由相鄰切線構成，因此**連續攤位無縫相接**，不會因為中心間距與格子尺寸兩套數值不匹配而出現縫隙或重疊；切線一律由框的起點加上總長的 i/N 算出，長排也不會累積浮點誤差。方向預設取框的長軸，可自由覆寫；換軸時編號起點回到該軸的第一端。排標籤與代碼前綴是任意字串，因此地支（`子`、`丑`…）或 `商` 這類區段不需特殊處理；前綴留空則沿用排標籤。
  - 框選後四角出現把手可微調範圍，並對已放好的攤位吸附（按住 Alt 暫停）。**調整框不算一步**，只有按下建立才寫進 layout，因此一次「復原」退掉的是整個排段。
  - 建立後面板不關閉：排標籤、前綴、補零位數、方向與編號起點都留著，**起始編號自動加上剛才的格數**，可以直接框下一段。K51 的 A 排因此是框 12 格（01–12）→ 直接框 20 格（13–32）→ 把起始編號改成 33 再框 12 格。
...
- 建立前會即時把框切成 N 格畫在畫布上並預覽代碼，並擋下跨排重複的攤位代碼。重複的排標籤不再是錯誤，而是併段。
