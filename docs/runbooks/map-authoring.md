...
- **「新增排／排段」一次產生一整段攤位**：輸入排標籤、起點與終點、格數、每格寬高與編號規則（前綴、起始編號、補零位數），工具依端點等距產生 slot。起點與終點是**第一格與最後一格的中心**。**排方向由端點自動判定**（橫向跨距大於縱向即為橫排），不另外輸入——兩個渲染器都用 `orientation` 決定排標籤放在哪一軸，與幾何矛盾的值會把標籤放到錯的位置。特殊情況可在建立後於排清單更改。排標籤與代碼前綴是任意字串，因此地支（`子`、`丑`…）或 `商` 這類區段不需特殊處理；前綴留空則沿用排標籤。
- **一個排標籤可以有多個排段**：填入既有排標籤時，這一段會**併入該排**，面板顯示「將加入既有 B 排（目前 N 格）」；填新標籤才建立新排。CWT*K51 的 B 區左右兩欄、被通道切成三段的 A 區、以及一排背對背的兩面攤位，都是同一個排標籤下的數個排段。**排的方向取第一段的方向**，所以同一排的各段方向不一致時，排標籤會落在錯的一軸。
- **編號遞減的排把起點放在號碼大的那一端**：編號一律從起點往終點遞增，所以「左邊 48、右邊 37」就是起點在右、終點在左，不需要另一套規則。
- 建立前會即時預覽將產生的代碼，並擋下跨排重複的攤位代碼。重複的排標籤不再是錯誤，而是併段。
- **錨點推算**：在「新增一排」面板按「開始標記錨點」後，於畫布空白處點選三格以上的中心，並在清單填入該格在這一排的編號（可不相鄰、不依序，例如第 1、5、12 格）。工具以最小平方法對兩軸擬合直線，外推出最低到最高編號之間的每一格，並顯示格數、起始編號與**最大偏差**——偏差大代表某個錨點點在別格上，應先修正再推算。三個是最少能互相矛盾的數量，兩個只能決定一條線而無法平均掉點選誤差。
  ...
  - 改動排標籤、代碼前綴、每格寬高、補零位數等任一欄位，或新增、移除、改動任一錨點編號，都會**丟棄既有草稿**；改變畫布尺寸時錨點與其他元素一起依比例縮放，草稿同樣丟棄，必須重新推算，避免畫面顯示新值卻寫入舊值。關閉「新增一排」面板會一併結束錨點標記，畫布隨即恢復框選。
