...
- **Mobile:** 760px 以下頂部保留品牌、活動與搜尋；工具選單收納字級、資料管理、使用說明。頁首層級高於地圖浮動控制與底部工作面板、低於模態對話框。使用說明固定在視窗頂部 56px、左右留 12px，長內容在面板內捲動。Escape 與選單外的按壓都會關閉它，關閉不吃掉那次地圖操作。日期／場館浮在地圖上方，底部「探索／行程」兩入口承載工作內容，篩選位於探索內。
