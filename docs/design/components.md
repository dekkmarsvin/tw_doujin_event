...
- **Area Switcher:** 桌機位置工具列、手機浮動工具顯示活動定義的場館名稱與目前展區標籤。選單第一項是「全區」，也是進站的預設——展區由攤位名單推導，直接落在第一個代碼上等於把讀者關進一個他還沒挑過的區塊。單一展區的場館空間不出現這一項。FF47 的展區全在同一個場館內，`areaMode` 為 `single`，因此固定顯示全館、不出現展區切換控制。
