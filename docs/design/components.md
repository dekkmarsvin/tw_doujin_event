...
- **Map Side Panel:** 桌機條件式覆蓋地圖右側，寬至多 330px、頂與右側 16px、高度隨內容縮合，最長保留底部 76px；標頭與操作留在上方，圖片及補充內容獨立捲動，不加高度動畫；無選取不留空欄。「開啟完整詳細資訊」下方提供「認領／管理資料」次要連結，帶入活動及社團，不以缺少補充資料推斷尚未認領。桌機日期／空間與來源摘要至少 12px，隨全域字級增大；深色文字對各自背景至少 4.5:1。X／Escape 取消選取及選取 URL，還原觸發焦點與未選取提示。760px 以下使用獨立摘要：回結果／取消選取、代碼與名稱、最多兩行介紹、加入行程／收藏／查看完整資訊。「取消選取」不留下收合列；把手收合保留選取。完整資訊 X／Escape／遮罩返回原摘要／詳情與觸發按鈕。摘要頂端的把手視覺上是一條細線，按壓範圍延伸到標題列留白處以維持 44px，回結果與取消選取仍在最上層。
