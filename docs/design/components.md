...
- **Desktop:** 探索／行程共用左欄（>1050px 為 296px，761–1050px 為 248px），頁籤不改導航模式。日期與場館工具位於地圖左上；頂部保留唯一搜尋、活動 h1、字級、資料管理與關於，基準 72px 並允許換行。活動資訊依內容分配寬度，名稱、日期及場館可換行，不截成省略號；窄桌機或放大字級時搜尋獨立一列。字級群組與資料管理、使用說明外框皆為 44px 高、7px 圓角；三段選中白底與陰影保留。
...
- **Map Side Panel:** 桌機條件式覆蓋地圖右側，寬至多 330px、頂與右側 16px、底部 76px；無選取不留空欄。X／Escape 取消選取及選取 URL，還原觸發焦點與未選取提示。760px 以下使用獨立摘要：回結果／取消選取、代碼與名稱、最多兩行介紹、加入行程／收藏／查看完整資訊。「取消選取」不留下收合列；把手收合保留選取。完整資訊 X／Escape／遮罩返回原摘要／詳情與觸發按鈕。摘要頂端的把手視覺上是一條細線，按壓範圍延伸到標題列留白處以維持 44px，回結果與取消選取仍在最上層。
