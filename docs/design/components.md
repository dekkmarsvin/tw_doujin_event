...
- **Style:** 桌機日期位於地圖左上浮動工具；手機沿用第二列日期與展區。active 日期使用底部 3px 珊瑚指示。
...
- **Area Switcher:** 桌機位置工具列、手機地圖標題顯示活動定義的場館名稱與目前展區標籤。FF47 的展區全在同一個場館內，`areaMode` 為 `single`，因此固定顯示全館、不出現展區切換控制。
...
- **Map Side Panel:** 桌機條件式覆蓋地圖右側，寬至多 330px、頂與右側 16px、底部 76px；無選取不留空欄。關閉保留選取與 URL，還原觸發焦點。760px 以下沿用底部詳細資訊頁籤。
...
- **Navigation Mode:** 啟用導航切到行程；切回探索保留導航投影。開始搜尋退出導航且保留其他篩選；退出入口固定於地圖底部。手機維持原工作面板流程。
