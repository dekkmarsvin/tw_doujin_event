...
- **Desktop:** 探索／行程共用左欄（>1050px 為 296px，761–1050px 為 248px），頁籤不改導航模式。日期與場館工具位於地圖左上；頂部保留唯一搜尋、活動 h1、字級、資料管理與關於，基準 72px 並允許換行。
...
- **Map Side Panel:** 桌機條件式覆蓋地圖右側，寬至多 330px、頂與右側 16px、底部 76px；無選取不留空欄。關閉保留選取與 URL，還原觸發焦點。760px 以下使用獨立摘要：回結果／收起、代碼與名稱、最多兩行介紹、加入行程／收藏／查看完整資訊。收起保留選取，完整資訊關閉回到原摘要按鈕。摘要頂端的把手視覺上是一條細線，按壓範圍延伸到標題列留白處以維持 44px，回結果與收起仍在最上層。
