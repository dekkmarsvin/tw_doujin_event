...
> 歷史範圍註記（2026-09-12）：本文記錄桌機改版當時的設計與驗收。所有「手機保留四頁籤／沿用原規則／原關閉語意」敘述已由 [ADR-0056](https://github.com/dekkmarsvin/tw_doujin_event/blob/4704e27ebd0ff88a6680356204b4306cf5314af8/docs/adr/0056-mobile-map-uses-workspace-and-selection-summary.md) 取代；現行手機為探索／行程兩入口、獨立摘要與收合預留矩形 fit，驗收見 [手機方案 B](https://github.com/dekkmarsvin/tw_doujin_event/blob/4704e27ebd0ff88a6680356204b4306cf5314af8/design-qa.md)。歷史測試數字不作為本次手機驗收。
...
- 決策優先序：本文件的目標行為 > 驗證文件中的候選方案 > 原提案與生成概念圖。2026-09-22 的窄桌機收欄決策見 [ADR-0055](https://github.com/dekkmarsvin/tw_doujin_event/blob/4704e27ebd0ff88a6680356204b4306cf5314af8/docs/adr/0055-desktop-map-uses-two-zones-and-overlay-details.md) 的 #204 補充；下方 248px 常駐欄是改版當時基準。現況仍以程式、[地圖契約](https://github.com/dekkmarsvin/tw_doujin_event/blob/4704e27ebd0ff88a6680356204b4306cf5314af8/docs/contracts/event-map.md)、[URL 契約](https://github.com/dekkmarsvin/tw_doujin_event/blob/4704e27ebd0ff88a6680356204b4306cf5314af8/docs/contracts/url-state.md)與 [DESIGN.md](https://github.com/dekkmarsvin/tw_doujin_event/blob/4704e27ebd0ff88a6680356204b4306cf5314af8/DESIGN.md)為準。
...
| 詳情 | `width:min(330px, calc(100% - 32px))`，相對地圖定位，左右最少 16px；頂 16px，高度隨內容縮合，最大高度為地圖高度減 92px，最長保留底 76px；標頭與操作固定於面板上方，其餘內容獨立捲動 |
...
| 詳情關閉或 Escape | 不移動、不縮放 | 取消選取與選取 URL；焦點還給觸發來源，程式還原焦點不再次宣告已選取（ADR-0063） |
...
selected 或 roving keyboard focus 具有完整代碼提示；鍵盤焦點存在時優先顯示焦點攤位，否則顯示 selected。兩者對讀者使用「已選取」一個詞彙；取消選取後的程式焦點還原例外，維持未選取提示，直到下一次使用者移動焦點。提示放在固定工具群中，以正常介面字級呈現，不放大格內文字或壓住相鄰格。selected 在全場概觀中保留標記，不等於詳情必須展開。
...
- Escape 在詳情及其子內容內取消選取；若有更上層完整資訊對話框，先關閉對話框並返回原詳情。詳情關閉按鈕與 Escape 都還原觸發元素；元素已不存在時依序退回同代碼 SVG 攤位、作用中左欄頁籤。
...
明說與既有 `DESIGN.md` 的 Three-Zone Rule 衝突，並擴充 [ADR-0001](https://github.com/dekkmarsvin/tw_doujin_event/blob/4704e27ebd0ff88a6680356204b4306cf5314af8/docs/adr/0001-adopt-webcatalog-patterns-selectively.md) 的版面參考範圍。保留 SVG、靜態閱讀與本機規劃資料等限制，不重寫舊 ADR 內文。

| 實作合併時同步 | 要更新的現況 |
|---|---|
| DESIGN.md／元件規格 | 欄寬、header 自適應、詳情覆蓋、手機邊界、工具層級與字級 |
| 活動地圖契約 | 新桌機 fit 矩形、初始化、完整代碼提示、文字省略與詳情定位 |
| URL 契約 | 關閉詳情取消選取；查看全場可收起並保留選取；完整資訊關閉返回原詳情；無新增參數 |
...
純計算擴充現有 `tests/map-viewport.test.mjs`；controller／renderer 行為加入相應模組測試，不以掃 CSS 字串代替行為測試。瀏覽器情境依專案既有工具記錄；後續正式程式交付執行[本機驗證](https://github.com/dekkmarsvin/tw_doujin_event/blob/4704e27ebd0ff88a6680356204b4306cf5314af8/docs/runbooks/local-development.md)規定的 gate。
...
本節記錄的尺寸矩陣由 `npm run test:browser:matrix` 重現；它自行處理 pinned FF47 staging 與 Vite 啟動。第一次執行前需 `npm run test:browser:install`。`npm run test:browser` 是 PR gate 的代表性尺寸子集，不代替此處的完整矩陣。`MAP_TEST_URL`、`BROWSER_CHANNEL`、`MAP_TEST_OUTPUT` 仍可覆寫；詳見[本機開發與驗證](https://github.com/dekkmarsvin/tw_doujin_event/blob/4704e27ebd0ff88a6680356204b4306cf5314af8/docs/runbooks/local-development.md#瀏覽器驗收)。一般 gate 為 `npm test`、`npm run lint`、`npx tsc --noEmit --incremental false`。
