...
桌機社團面板的「認領／管理資料」連到 `/circle?event=<eventId>&circle=<circleId>`。這只是表單目的地，不宣告是否已認領，也不授予擁有權。

- 索取社團登入連結時，請求的活動範圍與 `circleId` 一併帶到信件；僅保留可服務活動及該活動中存在的社團。信件固定指向本站 `/circle`，不接受任意轉址。
- 登入後使用受 session 保護的 `GET /api/circle/search?circle=<circleId>&event=<eventId>` 精確取得社團及可用佐證連結，不下載完整場刊。不存在時提供重新搜尋。
- 尚未持有該社團認領時預選認領表單；本人認領處理中時顯示進度，不重複送件；新送出的驗證碼與貼碼說明在清單更新後仍保留；本人已通過時提供資料管理入口，並將該編輯器列在其他社團之前。
