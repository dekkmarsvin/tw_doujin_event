...
- 讀取待審認領只要求有效登入與當下的管理者資格，不受登入後 24 小時的重新驗證期限限制；核准、婉拒與撤銷仍要求最近 24 小時內登入。
- 超過重新驗證期限的拒絕帶 `code: "admin_session_stale"`（HTTP 401），控制面據此辨識這一種拒絕，不比對訊息文字。
- **一個登入狀態鎖住整個面板，就在面板最上方說一次**：第一次被擋下就停用該面板所有需要重新驗證的控制項，並在標題之上顯示「管理功能被鎖定，需要重新登入。」與「登出並重新登入」。同一句話重複在每個表單下方沒有多給任何資訊；表單下方的那一行留給只有該表單能回報的錯誤。狀態只說讀者要做什麼，不揭露 24 小時期限這類實作細節。
