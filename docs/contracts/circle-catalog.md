...
- **`media` 為空是常態，不是例外。** 依 [ADR-0012](../adr/0012-first-party-sources-only.md) 縮圖只剩社團自填一個來源，因此三種密度都必須在沒有縮圖時收掉整個媒體區——清單卡不保留圖片欄、完整詳情改為單欄本文、地圖側欄不顯示代表圖按鈕——而不是留下空框或佔位圖。地圖攤位格的對應規則見[活動地圖契約](./event-map.md#縮放契約)。
...
依 [ADR-0012](../adr/0012-first-party-sources-only.md)，長期只保留**活動主辦官網**與**社團本人自填**兩類來源。

| `SourceContentType` | 長期地位 |
|---|---|
| `official` | 保留。主辦官網供應配置與活動事實。 |
| `circle` | 保留。社團自填供應內容。 |
| `catalog` | **過渡類別。** 目前的生產者是社群維護的試算表；退場完成後不再有新內容進來，型別只保留給歷史資料。 |
| `social`、`media` | 只作為社團登錄的連結標示，不作為抓取來源。`media` 已無生產者——工作簿縮圖索引退場後，快照不再帶 `thumbnail`，縮圖只來自社團自填的 overlay。 |

退場期間官網與工作簿並存。同一攤位兩邊名稱不一致時 **fail closed 並輸出人工裁決報告**，比照 identity registry 的既有做法，不自動採信任一方。
