...
公開活動介紹使用 `/events/<eventId>/`，社團在該活動的介紹使用 `/events/<eventId>/circles/<circleId>/`。它們是在 build 時從同一份已發布活動／reviewed base 產生的 `index.html`，不是 SPA fallback。社團頁集中呈現跨日、多攤位與已移動／已取消狀態；每筆配置以既有 query URL 回到正確日期、場地與攤位。未知靜態路徑沿用真實 404。
...
- 社團介紹的 title 為「社團名稱｜活動短稱 M/D 攤位、攤位｜場刊 Map」，只取有效配置：日期與攤位取最早一個有效配置那一天。完全沒有有效配置時，改用最早一筆失效配置的活動日與攤位並附「已移動攤位」或「已取消參展」，網址與 canonical 不變。description 依活動日列出全部攤位，失效配置帶同樣的狀態用詞，日期寫成完整年月日；活動介紹的 description 也使用完整年月日。社團介紹的攤位列不論活動以 ISO 或月日標籤發布，日期都寫成「11月7日（六）」。活動介紹名單中同名的社團以同一組日期與攤位區分。Reader 選取社團時以同一份已審核配置產生相同的 title 與 description（[circle catalog 契約](./circle-catalog.md)的失效配置呈現）。
...
| `day` | 活動 | 活動日，永遠寫出 |
| `area` | 活動 | 展區，永遠寫出。讀取時接受 legacy 別名 `hall`。另接受讀者端的 `ALL`（目前場地的全部展區）|
...
除 `event`、`day`、`area` 外，**參數在等於預設值時從 URL 移除**，不留下無意義的殘留條件；多場地活動的 `venueSpaceId` 是例外，必須寫出以消除 `area` 歸屬歧義。
...
- **defaults 從 event definition 推導。** `day` 與 genre 預設取活動定義的第一筆，不在 codec 內硬編碼 FF47 的 `1` 或「全部類別」。`area` 的預設是**該場地的全區**：由第一個展區決定落在哪個場地，再開在那個場地的全部展區上。
- **`ALL` 是讀者端的展區代碼，不是活動資料裡的一筆。** 展區由主辦的攤位名單推導，因此活動的 `areas` 只會有名單裡出現過的代碼，沒有一個代表「全部」；讀者自行補上 `ALL`。FF47 的定義自行宣告了同名展區，兩者因此重合而不衝突。`ALL` 一律指**目前場地**的全部展區，不是整場活動——地圖 artifact 的單位是「活動日 × 場地」，別的場地的攤位在目前這張圖上沒有座標。只有一個展區的場地不提供 `ALL`，`?area=ALL` 在那裡正規化成那個展區。
- **場地與展區成對驗證。** `area` 是活動分區，`venueSpaceId` 是 pinned 場地，兩者不得互換。無效的 space 或不屬於該 space 的 area 一律回到該活動的預設 assignment；單一場地活動會移除殘留的 `venueSpaceId`。`ALL` 屬於每一個場地，本身不指定場地，因此寫出時的 `venueSpaceId` 取自讀者目前的場地狀態，不由 `area` 反推。
- **不提供展區切換的活動，`area` 只有一個可達值。** 單一場地的活動不出現展區切換（見 [`event-map.md`](event-map.md)），因此 `?area=` 指名個別展區時放寬成全區，而不是把讀者留在一個他沒有控制項可以解除的篩選裡。活動、日期與選取不受影響，篩選只會放寬不會收窄，原本看得到的東西不會消失。
