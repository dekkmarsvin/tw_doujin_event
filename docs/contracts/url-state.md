...
- 首頁提供活動地圖與活動介紹的 href；每個活動介紹列出全部有配置的社團 href，不依賴「載入更多」。多日活動的名單依活動日分段（依日曆日期排序，標題為「11月7日（六）」格式與當日社團數），社團在每個有配置的活動日各列一次、連到同一張社團頁；當天只有失效配置時附「已移動攤位」或「已取消參展」。單日活動只有一份名單。Reader 清單的一般點擊保持地圖選取，修改鍵／新分頁可開啟靜態社團頁。
