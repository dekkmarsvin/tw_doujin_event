...
- 桌機的「查看全場」收起詳情、恢復 fit，保留選取、分享 URL 與規劃資料；關閉詳情只收起浮層、不移動視域。再次選取同攤位可重新開啟。Escape 只在詳情內關閉，焦點還給觸發來源，來源不存在則回 SVG 入口或作用中的左欄頁籤。手機收起同樣保留選取、URL 與視域，顯示可重開摘要的社團列；回結果恢復列表捲動及焦點，完整資訊沿用 dialog。
