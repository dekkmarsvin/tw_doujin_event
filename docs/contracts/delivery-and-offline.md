...
介紹頁由 Pages 靜態直送，不經 Function，不新增資料寫入。Event JSON-LD 僅使用可解析的活動日日期與既有場館／主辦名稱、網址，活動有別稱時以 `alternateName` 列出；`image` 使用下段的品牌分享圖。無資料的地址、售票、表演者、活動狀態與開場時間省略，不保證 rich result 資格。
