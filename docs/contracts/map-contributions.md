...
選取兩排以上的直排攤位後可一次套用共同的上界、下界與內部分隔線。共同邊界取自整個選取的外緣，每一排再以無縫等分重切，因此一排的分隔線與下一排的分隔線落在同一條線上，不會因為各自四捨五入格高而漂開。**每一格保留原本的 X 與寬度**，排與排之間的走道不變；未選取的元件完全不動。

畫布範圍、最小尺寸與「下界必須大於上界」在共同邊界層級檢查一次，不逐格各自夾限。不適用時動作停用並在旁邊說明原因：不足兩排、全部屬於同一排、各排格數不同（列出實際格數）、選取中含橫排攤位，或共同高度放不下該格數。代碼、順序與編號方向不受影響——這個動作只改幾何。一次套用是一個歷史步驟，一次復原即可還原整批。
