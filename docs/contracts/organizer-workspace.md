...
- 動作回饋遵循 [Action Feedback](../design/components.md#action-feedback)：切換引導任務不沿用前一任務的驗證或儲存訊息；建置冊儲存活動、場館或匯入後，重新取得已存版本仍保留當次成功回饋，再編輯或發動下一動作才清除。已存草稿的正規化值同步回未修改的表單；有未儲存輸入時，不因重新整理而更新它的 `expectedVersion`。本機記住的活動先與可見清單核對，已移除活動及離開後才回來的讀取結果不干擾目前工作。
