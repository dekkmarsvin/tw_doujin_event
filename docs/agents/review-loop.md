...
純流程／說明文件變更依[本機開發與驗證的純文件檢查](../runbooks/local-development.md#純文件變更)驗證，在 PR 註明未跑程式測試的理由。既有 required CI checks 照常執行，不因本機驗證減量而關閉、繞過或削弱；必要的授權、資料完整性、snapshot、恢復與發布驗證也不得移除。驗證結果要能對應待交付版本，不能把未執行、失敗或 skipped 的必要檢查寫成通過。
