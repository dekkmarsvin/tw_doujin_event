...
const precache = files.filter((path) => PRECACHE_PATTERNS.some((pattern) => pattern.test(path))).sort();
...
if (!precache.some((path) => path.startsWith("/assets/") && path.endsWith(".js"))) throw new Error("The build produced no application script to precache.");
