  ...
  for (const [width, height, scales] of MATRIX) {
    for (const scale of scales) {
      ...
      if (width > 760) {
        ...
        const handle = await page.getByRole("button", { name: "縮小工作面板", exact: true }).boundingBox();
        assert.equal(await page.evaluate(([x, y]) => document.elementFromPoint(x, y)?.getAttribute("aria-label"), [handle.x + handle.width / 2, handle.y + 43]), "縮小工作面板", "the handle keeps a 44px press target");
