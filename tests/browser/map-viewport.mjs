...
const state = (page) => page.evaluate(() => {
  ...
  return {
    ...
    mapLabel: rect(document.querySelector('[class*="mobileMapSelection"]')), mapMarker: rect(document.querySelector('[class*="mobileMapMarker"]')),
...
// The design report quotes one effective map height per size and scale. It is
// derived here so the number in the report and the number this script asserts
// can never drift apart, and so the matrix can be regenerated by rerunning it.
async function recordMobileMatrix(page, width, height, scale, measured) {
  const actions = await page.locator('[aria-label="已選社團摘要"] button').all();
  const boxes = await Promise.all(actions.map((action) => action.boundingBox()));
  report.matrix.push({
    width, height, scale,
    mapHeight: measured.dock.top - measured.tools[0].bottom - 32,
    actionCount: boxes.filter(Boolean).length,
    actionsClear: boxes.every((box) => !box || box.y + box.height <= measured.nav.top + 1),
    overflow: measured.bodyWidth > measured.width,
    selectedInside: measured.selected.top >= measured.tools[0].bottom
      && measured.selected.bottom <= measured.dock.top
      && measured.selected.right <= measured.controls.left,
  });
}
...
try {
  for (const [width, height, scales] of MATRIX) {
    for (const scale of scales) {
      ...
      if (width > 760) {
        ...
        assert.ok(selected.mapLabel.right <= selected.controls.left, "the booth label clears the zoom controls");
        assert.ok(selected.mapLabel.left >= selected.map.left, "the booth label stays on the map");
        assert.ok(Math.abs(selected.mapMarker.x + selected.mapMarker.width / 2 - (selected.selected.x + selected.selected.width / 2)) < 2, "the marker sits on the selected booth");
        assert.ok(Math.abs(selected.mapLabel.x + selected.mapLabel.width / 2 - (selected.mapMarker.x + selected.mapMarker.width / 2)) <= 42, "the booth label stays under its marker");
        await recordMobileMatrix(page, width, height, scale, selected);
        if (width === 390 && scale === "standard") assert.ok(selected.dock.top - selected.tools[0].bottom - 32 >= 240);
        const handle = await page.getByRole("button", { name: "完整展開工作面板", exact: true }).boundingBox();
        assert.equal(await page.evaluate(([x, y]) => document.elementFromPoint(x, y)?.getAttribute("aria-label"), [handle.x + handle.width / 2, handle.y + 43]), "完整展開工作面板", "the handle keeps a 44px press target");
        const selectedUrl = page.url();
        await page.getByRole("button", { name: "查看完整資訊", exact: true }).click();
        assert.equal(await page.locator("main").getAttribute("data-mobile-sheet-level"), "full");
        assert.equal(await page.getByRole("dialog").count(), 0, "mobile full details stay in the sheet");
        await page.locator('[data-embedded="true"]').waitFor();
        await capture(page, `${prefix}-full-details`);
  ...
  const summary = mobile.locator('[aria-label="已選社團摘要"]');
  const banner = mobile.locator('[class*="navigationBanner"]');
  // A circle holding two adjacent booths must not swap the reader's booth for
  // its sibling when navigation starts.
  await mobile.locator("[data-slot-code=A01]").click();
  await pause(mobile);
  await summary.getByRole("button", { name: "加入行程", exact: true }).click();
  assert.equal(await summary.getByRole("status").innerText(), "已加入行程", "adding to the itinerary says so");
  const mobileSelectedUrl = mobile.url();
  await mobile.getByRole("group", { name: "行動版工作區" }).getByRole("button", { name: /^行程/ }).click();
  await mobile.getByRole("button", { name: "開始導航", exact: true }).click();
  await pause(mobile);
  assert.match(await banner.innerText(), /目前目標 A01/, "the banner names the booth the reader opened");
  assert.equal(await summary.locator("strong").innerText(), "A01", "the summary names the same booth");
  assert.equal(new URL(mobile.url()).searchParams.get("selectedBooth"), "A01", "navigation leaves the shared booth alone");
  await banner.getByRole("button", { name: "退出", exact: true }).click();
  await pause(mobile);

  // The tools menu owes a reader both exits.
  const toolsMenu = mobile.locator("details");
  await toolsMenu.locator("summary").click();
  await mobile.getByRole("button", { name: "最大字級", exact: true }).focus();
  await mobile.keyboard.press("Escape");
  assert.equal(await toolsMenu.evaluate((node) => node.open), false, "Escape closes the tools menu");
  assert.equal(await toolsMenu.locator("summary").evaluate((node) => node === document.activeElement), true);
  await pause(mobile);
  const mapBefore = await state(mobile);
  await toolsMenu.locator("summary").click();
  await mobile.locator(".map").click({ position: { x: 20, y: 150 } });
  await pause(mobile);
  assert.equal(await toolsMenu.evaluate((node) => node.open), false, "a press outside closes the tools menu");
  const afterMenu = await state(mobile);
  assert.deepEqual(afterMenu.offset, mapBefore.offset, "closing the menu leaves the map where it was");
  assert.equal(afterMenu.zoom, mapBefore.zoom);

  // A day the selection cannot survive returns the reader to the workspace.
  await mobile.locator("[data-slot-code=A01]").click();
  await pause(mobile);
  await mobile.locator('[class*="dateSelect"] select').selectOption("2");
  await mobile.locator(".floor").waitFor();
  await pause(mobile);
  assert.equal(await mobile.locator("main").getAttribute("data-mobile-summary"), null, "an invalid selection leaves no empty summary");
  assert.equal(await mobile.locator("main").getAttribute("data-mobile-sheet-level"), "peek");
  assert.equal(new URL(mobile.url()).searchParams.get("selectedBooth"), null);

  // A shared link reopens the summary, and the breakpoint carries it both ways.
  await mobile.goto(mobileSelectedUrl);
  await summary.waitFor();
  await pause(mobile);
  const restored = await state(mobile);
  assert.equal(await summary.locator("strong").innerText(), "A01");
  await mobile.setViewportSize({ width: 761, height: 844 });
  await pause(mobile);
  assert.equal(await detail(mobile).count(), 1, "a selection crossing to desktop opens the detail overlay");
  await mobile.setViewportSize({ width: 390, height: 844 });
  await pause(mobile);
  assert.equal(await summary.count(), 1, "and crossing back reopens the summary");
  assert.equal((await state(mobile)).zoom, restored.zoom, "the breakpoint does not refit a restored selection");
  await capture(mobile, "mobile-flows-390-844-standard");
  await mobile.close();
  report.cases.push({ name: "mobile-flows", passed: true });
