...
import { assertEventDataPinIdentity, parseEventDataPin, rawFileUrl } from "../scripts/event-data-pin-utils.mjs";
...
test("event data is pinned to a full commit with per-file hashes", () => {
  ...
  assert.deepEqual(pin.files.map((file) => file.path), ["event.json", "official-booths.json", "map.json", "reference-data-pin.json"]);
...
test("pin parsing rejects traversal, short commits and unknown schemas", () => {
  ...
  assert.throws(() => assertEventDataPinIdentity(parseEventDataPin({ ...pin, eventId: "other-event" }), "ff47"), /identity mismatch/);
