...
export default function PlanningDisplayControls({ value, groups, onApply }: {
  ...
  return <div className={styles.wrap}>
    <button type="button" className={styles.trigger} onClick={() => { setDraft(value); setOpen(true); }}>行程、收藏與顯示{activeCount > 0 && <span>{activeCount}</span>}</button>
    {open && <section ref={panelRef} className={styles.panel} role="dialog" aria-modal="true" aria-label="行程、收藏與顯示設定" tabIndex={-1}>
      <header><b>行程、收藏與顯示</b><button type="button" onClick={cancel} aria-label="取消並關閉"><UiIcon name="close" /></button></header>
      ...
      <fieldset><legend>資訊密度</legend><div className={styles.segments}><button type="button" className={draft.density === "compact" ? styles.active : ""} onClick={() => setDraft({ ...draft, density: "compact" })}>精簡</button><button type="button" className={draft.density === "informative" ? styles.active : ""} onClick={() => setDraft({ ...draft, density: "informative" })}>資訊</button></div></fieldset>
      <label>每筆媒體<select value={draft.mediaCount} onChange={(event) => setDraft({ ...draft, mediaCount: Number(event.target.value) as 0 | 1 | 3 })}><option value={0}>不顯示</option><option value={1}>1 張</option><option value={3}>最多 3 張</option></select><small>只顯示社團自行提供且可追溯原始連結的圖片；沒有縮圖的社團維持文字卡。</small></label>
      <footer><button type="button" onClick={cancel}>取消</button><button type="button" className={styles.apply} onClick={() => { onApply(draft); setOpen(false); }}>套用</button></footer>
    </section>}
  </div>;
}
