...
export default function PlanningDisplayControls({ value, groups, onApply }: {
  ...
  return <div className={styles.wrap}>
    ...
    {open && <section ref={panelRef} className={styles.panel} role="dialog" aria-modal="true" aria-label="行程、收藏與顯示設定" tabIndex={-1}>
      ...
      <label>每筆媒體<select value={draft.mediaCount} onChange={(event) => setDraft({ ...draft, mediaCount: Number(event.target.value) as 0 | 1 | 3 })}><option value={0}>不顯示</option><option value={1}>1 張</option><option value={3}>最多 3 張</option></select></label>
