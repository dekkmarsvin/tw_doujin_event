...
export default function PlanningTools({ eventId }: { eventId: string }) {
  ...
  const orphanFavorites = document.favorites.filter((favorite) => favorite.eventId === eventId && !isKnownCircleId(favorite.circleId, favorite.eventId));
  const orphanPlans = document.visitPlans.filter((entry) => entry.eventId === eventId && !isKnownCircleId(entry.circleId, entry.eventId));
  ...
  return <>
    ...
    {open && createPortal(<div className={styles.backdrop} role="presentation" onPointerDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}><section ref={dialogRef} className={styles.dialog} role="dialog" aria-modal="true" aria-labelledby="planning-tools-title" tabIndex={-1}>
      ...
      <section className={`${styles.section} ${styles.danger}`}><div><h3>清除所有規劃資料</h3><p>會移除 {document.favorites.length} 筆收藏、{memoCount} 筆備註、{document.visitPlans.length} 筆行程與 {document.favoriteGroups.length} 個群組。{unsupportedRaw && "也會刪除無法讀取的舊資料。"}</p></div>{confirmClear ? <div className={styles.confirmRow}><span>確定要永久清除這台裝置上的資料？</span><button onClick={() => { replace(EMPTY_PLANNING_DOCUMENT); setConfirmClear(false); setNotice("已清除所有規劃資料。"); }}>確定清除</button><button onClick={() => setConfirmClear(false)}>取消</button></div> : <button onClick={() => setConfirmClear(true)}>清除資料…</button>}</section>
