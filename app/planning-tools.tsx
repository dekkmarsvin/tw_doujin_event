...
import { CIRCLE_RECORDS_BY_ID } from "./circle-records";
...
export default function PlanningTools() {
  ...
  const orphanFavorites = document.favorites.filter((favorite) => favorite.eventId === FF47_EVENT_ID && !CIRCLE_RECORDS_BY_ID.has(favorite.circleId));
  const orphanPlans = document.visitPlans.filter((entry) => entry.eventId === FF47_EVENT_ID && !CIRCLE_RECORDS_BY_ID.has(entry.circleId));

  return <>
    <button className={styles.launcher} onClick={() => setOpen(true)}>規劃資料</button>
    {open && <div className={styles.backdrop} role="presentation" onPointerDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}><section ref={dialogRef} className={styles.dialog} role="dialog" aria-modal="true" aria-labelledby="planning-tools-title" tabIndex={-1}>
      ...
      {(orphanFavorites.length > 0 || orphanPlans.length > 0) && <section className={styles.section}><div><h3>目前無法匹配的規劃資料</h3><p>社團可能已取消、移動或不在目前場刊；備註與行程會保留，您仍可匯出或逐筆移除。</p></div><div className={styles.orphanList}>{orphanFavorites.map((favorite) => <div key={`favorite-${favorite.circleId}`}><span><b>{favorite.circleId}</b><small>收藏{favorite.memo ? ` · 備註：${favorite.memo}` : ""}</small></span><button onClick={() => update((current) => toggleFavorite(current, favorite.eventId, favorite.circleId))}>移除收藏</button></div>)}{orphanPlans.map((entry) => <div key={`plan-${entry.day}-${entry.circleId}`}><span><b>{entry.circleId}</b><small>DAY {entry.day} · {entry.status === "next" ? "下一站" : entry.status === "visited" ? "已走訪" : "待前往"}</small></span><button onClick={() => update((current) => removeFromVisitPlan(current, entry.eventId, entry.day, entry.circleId))}>移除行程</button></div>)}</div></section>}
