import { previewE2eAuthorized, previewRecipientAllowed, repositoryFor } from "../../_portal";
...
export const onRequestGet: PagesFunction<PortalEnv> = async ({ request, env }) => {
  ...
  if (!previewRecipientAllowed(env, email)) return json({ error: "recipient is not allowlisted" }, 403);
